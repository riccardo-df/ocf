...
void ForestOrdered::initInternal() {
  ...
  if (min_node_size == 0) {
    min_node_size = DEFAULT_MIN_NODE_SIZE_ORDERED;
  }

  // Sorting data if memory saving mode.
...
void ForestOrdered::allocatePredictMemory() {
...
void ForestOrdered::predictInternal(size_t sample_idx) {
  if (predict_all || prediction_type == TERMINALNODES) { // Getting all tree predictions.
...
void ForestOrdered::computePredictionErrorInternal() {
...
} // CURRENTLY IGNORED IN R!

// #nocov start
void ForestOrdered::writeOutputInternal() {
  if (verbose_out) {
    *verbose_out << "Tree type:                         " << "Ordered" << std::endl;
  }
}

void ForestOrdered::writeConfusionFile() {
  // Opening confusion file for writing.
...
void ForestOrdered::writePredictionFile() {
  // Opening prediction file for writing.
...
void ForestOrdered::saveToFileInternal(std::ofstream& outfile) {
  // Writing num_variables.
  outfile.write((char*) &num_independent_variables, sizeof(num_independent_variables));

  // Writing treetype.
  TreeType treetype = TREE_ORDERED;
  outfile.write((char*) &treetype, sizeof(treetype));
}

void ForestOrdered::loadFromFileInternal(std::ifstream& infile) {
  ...
  if (treetype != TREE_ORDERED) {
    throw std::runtime_error("Wrong treetype. Loaded file is not an ordered forest.");
  }
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    if (num_variables_saved != num_independent_variables) {
      throw std::runtime_error("Number of covariates in data does not match with the loaded forest.");
    }
...
double ForestOrdered::getTreePrediction(size_t tree_idx, size_t sample_idx) const {
  const auto& tree = dynamic_cast<const TreeOrdered&>(*trees[tree_idx]);
  return tree.getPrediction(sample_idx);
}

size_t ForestOrdered::getTreePredictionTerminalNodeID(size_t tree_idx, size_t sample_idx) const {
  const auto& tree = dynamic_cast<const TreeOrdered&>(*trees[tree_idx]);
