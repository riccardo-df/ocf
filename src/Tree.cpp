...
Tree::Tree() :
    ...
    case_weights(0), manual_inbag(0), oob_sampleIDs(0), holdout(false), keep_inbag(false), data(0), regularization_factor(0),
    regularization_usedepth(false), split_varIDs_used(0), variable_importance(0), importance_mode(DEFAULT_IMPORTANCE_MODE),
    sample_with_replacement(true), sample_fraction(0), memory_saving_splitting(false), splitrule(DEFAULT_SPLITRULE),
    alpha(DEFAULT_ALPHA), minprop(DEFAULT_MINPROP), num_random_splits(DEFAULT_NUM_RANDOM_SPLITS), max_depth(DEFAULT_MAXDEPTH), 
    depth(0), last_left_nodeID(0) {
}

Tree::Tree(std::vector<std::vector<size_t>>& child_nodeIDs, std::vector<size_t>& split_varIDs,
           std::vector<double>& split_values) : mtry(0), num_samples(0), num_samples_oob(0), min_node_size(0), 
           deterministic_varIDs(0), split_select_weights(0), case_weights(0), manual_inbag(0), split_varIDs(split_varIDs), 
           split_values(split_values), child_nodeIDs(child_nodeIDs), oob_sampleIDs(0), holdout(false), keep_inbag(false), 
           data(0), regularization_factor(0), regularization_usedepth(false), split_varIDs_used(0), variable_importance(0),
...
void Tree::init(const Data* data, unsigned int mtry, size_t num_samples, unsigned int seed, 
                std::vector<size_t>* deterministic_varIDs, std::vector<double>* split_select_weights,
                ImportanceMode importance_mode, unsigned int min_node_size, bool sample_with_replacement, 
  ...
  // Creating root node, assigning bootstrap sample and oob samples.
  child_nodeIDs.push_back(std::vector<size_t>());
  child_nodeIDs.push_back(std::vector<size_t>());
  createEmptyNode();

  // Initializing random number generator and setting seed.
...
void Tree::grow(std::vector<double>* variable_importance) {
  // Allocating memory for tree growing.
  allocateMemory();
  ...
  while (num_open_nodes > 0) {
    ...
    if (is_terminal_node) {
      ...
      if (i >= last_left_nodeID) {
        /* If new level, increase depth. left_node saves left-most node in current level, new level reached if 
           that node is splitted. */
  ...
  // Deleting sampleID vector to save memory.
  sampleIDs.clear();
...
void Tree::predict(const Data* prediction_data, bool oob_prediction) {
  ...
  // For each sample, start in root and drop down the tree and return final value.
  for (size_t i = 0; i < num_samples_predict; ++i) {
    ...
    while (1) {
      ...
      if (prediction_data->isOrderedVariable(split_varID)) {
        if (value <= split_values[nodeID]) { // Move to left child.
        ...
        if (!(splitID & (1ULL << factorID))) { // Move to left child.
          nodeID = child_nodeIDs[0][nodeID];
        } else { // Move to right child.
          nodeID = child_nodeIDs[1][nodeID];
        }
...
void Tree::computePermutationImportance(std::vector<double>& forest_importance, std::vector<double>& forest_variance,
  ...
  // Computing normal prediction accuracy for each tree. Predictions already computed.
  ...
  // Reserving space for permutations, initializing with oob_sampleIDs.
  std::vector<size_t> permutations(oob_sampleIDs);

  // Randomly permuting for all covariates.
  for (size_t i = 0; i < num_independent_variables; ++i) {

    // Checking whether the i-th variable is used in the tree.
    bool isused = false;
    ...
    if (isused) {
      // Permuting and computing prediction accuracy again for this permutation and save difference.
...
void Tree::createPossibleSplitVarSubset(std::vector<size_t>& result) {
  ...
  // Randomly adding non-deterministic variables (according to weights if needed).
  ...
  // Always using deterministic variables.
...
bool Tree::splitNode(size_t nodeID) {
  // Selecting random subset of variables to possibly split at.
  std::vector<size_t> possible_split_varIDs;
  createPossibleSplitVarSubset(possible_split_varIDs);

  // Calling subclass method, sets split_varIDs and split_values.
  bool stop = splitNodeInternal(nodeID, possible_split_varIDs); 
  
  if (stop) { // Terminal node
    return true;
  }
  ...
  // Saving non-permuted variable for prediction.
  ...
  if (data->isOrderedVariable(split_varID)) { // Ordered: left is <= splitval and right is > splitval.
    size_t pos = start_pos[nodeID];
    
    while (pos < start_pos[right_child_nodeID]) {
      size_t sampleID = sampleIDs[pos];
      if (data->get_x(sampleID, split_varID) <= split_value) {  // If going to left, do nothing.
        ++pos;
      } else { // If going to right, move to right end.
        --start_pos[right_child_nodeID];
        std::swap(sampleIDs[pos], sampleIDs[start_pos[right_child_nodeID]]);
      }
    }
  } else { // Unordered: If bit at position is 1 -> right, 0 -> left.
    size_t pos = start_pos[nodeID];
    while (pos < start_pos[right_child_nodeID]) {
      size_t sampleID = sampleIDs[pos];
      ...
      // Left if 0 found at position factorID.
      if (!(splitID & (1ULL << factorID))) { // If going to left, do nothing.
        ++pos;
      } else { // If going to right, move to right end.
        --start_pos[right_child_nodeID];
        std::swap(sampleIDs[pos], sampleIDs[start_pos[right_child_nodeID]]);
...
size_t Tree::dropDownSamplePermuted(size_t permuted_varID, size_t sampleID, size_t permuted_sampleID) {
  // Starting in root and dropping down.
  size_t nodeID = 0;
  while (child_nodeIDs[0][nodeID] != 0 || child_nodeIDs[1][nodeID] != 0) {

    // Permuting if variable is permutation variable.
    size_t split_varID = split_varIDs[nodeID];
...
void Tree::permuteAndPredictOobSamples(size_t permuted_varID, std::vector<size_t>& permutations) {
  // Permuting OOB sample
  std::shuffle(permutations.begin(), permutations.end(), random_number_generator);

  // For each sample, dropping down the tree and adding prediction.
...
void Tree::bootstrap() {
  ...
  // Starting with all samples OOB.
  inbag_counts.resize(num_samples, 0);

  // Drawing num_samples samples with replacement (num_samples_inbag out of n) as inbag and mark as not OOB.
...
void Tree::bootstrapWeighted() {
  ...
  // Drawing num_samples samples with replacement (n out of n) as inbag and mark as not OOB.
  for (size_t s = 0; s < num_samples_inbag; ++s) {
  ...
  // Saving OOB samples. In holdout mode these are the cases with 0 weight.
  if (holdout) {
...
void Tree::setManualInbag() {
  // Selecting observation as specified in manual_inbag vector.
