...
Forest::Forest() :
    ...
    num_samples(0), prediction_mode(false), memory_mode(MEM_DOUBLE), sample_with_replacement(true), 
    ...
    num_random_splits(DEFAULT_NUM_RANDOM_SPLITS), max_depth(DEFAULT_MAXDEPTH), alpha(DEFAULT_ALPHA), minprop(DEFAULT_MINPROP),
    num_threads(DEFAULT_NUM_THREADS), data { }, overall_prediction_error(NAN), importance_mode(DEFAULT_IMPORTANCE_MODE), 
    regularization_usedepth(false), progress(0) {
}

// #nocov start
void Forest::initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, unsigned int mtry,
    ...
    PredictionType prediction_type, unsigned int num_random_splits, unsigned int max_depth,
  ...
  init(loadDataFromFile(input_file), mtry, output_prefix, num_trees, seed, num_threads, importance_mode,
      ...
      false, max_depth, regularization_factor, regularization_usedepth, alpha_balance);

  if (prediction_mode) {
    loadFromFile(load_forest_filename);
  }
  
  // Setting variables to be always considered for splitting.
  if (!always_split_variable_names.empty()) {
    setAlwaysSplitVariables(always_split_variable_names);
  }

  // Loading split select weights from file.
  if (!split_select_weights_file.empty()) {
    ...
    if (split_select_weights[0].size() != num_independent_variables) {
      throw std::runtime_error("Number of split select weights is not equal to number of covariates.");
    }
    
    setSplitWeightVector(split_select_weights);
  }

  // Loading case weights from file.
  ...
  // Sampling from non-zero weights in holdout mode.
  ...
  // Checking if all categorical variables are coded in integers starting at 1.
...
void Forest::initR(std::unique_ptr<Data> input_data, unsigned int mtry, unsigned int num_trees, std::ostream* verbose_out, unsigned int seed,
    ...
    unsigned int num_random_splits, bool order_snps, unsigned int max_depth, const std::vector<double>& regularization_factor,
  ...
  init(std::move(input_data), mtry, "", num_trees, seed, num_threads, importance_mode, min_node_size,
      ...
      regularization_factor, regularization_usedepth, alpha_balance);
  ...
  // Setting split select weights.
  if (!split_select_weights.empty()) {
    setSplitWeightVector(split_select_weights);
  }

  // Setting case weights.
  ...
  // Setting manual inbag.
  if (!manual_inbag.empty()) {
    this->manual_inbag = manual_inbag;
  }

  // Keeping inbag counts.
  this->keep_inbag = keep_inbag;
}

void Forest::init(std::unique_ptr<Data> input_data, unsigned int mtry, std::string output_prefix,
    ...
    double alpha, double minprop, bool holdout, PredictionType prediction_type, unsigned int num_random_splits, bool order_snps,
    unsigned int max_depth, const std::vector<double>& regularization_factor, bool regularization_usedepth,
    std::vector<double>& alpha_balance) {
  
  // Initializing data with memory mode.
  this->data = std::move(input_data);

  // Initializing random number generator and set seed.
  if (seed == 0) {
  ...
  // Setting unordered factor variables.
  if (!prediction_mode) {
  ...
  // Initializing manual inbag.
  manual_inbag.push_back(std::vector<size_t>());

  // Checking if mtry is in valid range.
  if (this->mtry > num_independent_variables) {
    throw std::runtime_error("mtry can not be larger than number of covariates.");
  }

  // Checking if any observations samples.
  ...
  if (regularization_factor.size() > 0) {
    if (regularization_factor.size() == 1 && num_independent_variables > 1) {
      ...
      throw std::runtime_error("Use 1 or p (the number of covariates) regularization factors.");
    }

    // Setting all variables to not used.
...
void Forest::writeOutput() {
  ...
  if (verbose_out) {
    ...
    *verbose_out << "Number of covariates:              " << num_independent_variables << std::endl;
...
void Forest::writeImportanceFile() {
  // Opening importance file for writing.
...
void Forest::saveToFile() {
  ...
  // Writing covariates' names.
  unsigned int num_dependent_variables = dependent_variable_names.size();
...
void Forest::grow() {
  ...
  // Calling special grow functions in the subclass.
  growInternal(); 

  // Initializing trees and creating a seed for each tree based on main seed.
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    // Getting split select weights for tree.
  ...
  // Growing trees in multiple threads.
#ifdef OLD_WIN_R_BUILD
  // #nocov start
  progress = 0;
  ...
  // Initializing importance per thread.
  ...
  // Dividing importance by number of trees.
  if (importance_mode == IMP_GINI) {
...
void Forest::predict() { // Predicts trees in multiple threads and join the threads with the main thread.
#ifdef OLD_WIN_R_BUILD
  // #nocov start
  progress = 0;
  clock_t start_time = clock();
...
void Forest::computePredictionError() {
  ...
  // Calling special function for subclasses.
  computePredictionErrorInternal();
} 

void Forest::computePermutationImportance() {  // Computes tree permutation importance in multiple threads.
#ifdef OLD_WIN_R_BUILD
  // #nocov start
  progress = 0;
  clock_t start_time = clock();
  ...
  // Initializing importance and variance.
  variable_importance.resize(num_independent_variables, 0);
  ...
  // Summing thread casewise importances.
  if (importance_mode == IMP_PERM_CASEWISE) {
    variable_importance_casewise.resize(num_independent_variables * num_samples, 0);
  ...
  for (size_t i = 0; i < variable_importance.size(); ++i) {
    ...
    // Normalizing by variance for scaled permutation importance.
    if (importance_mode == IMP_PERM_BREIMAN || importance_mode == IMP_PERM_LIAW) {
      if (variance[i] != 0) {
...
#ifndef OLD_WIN_R_BUILD
void Forest::growTreesInThread(unsigned int thread_idx, std::vector<double>* variable_importance) {
...
void Forest::predictTreesInThread(unsigned int thread_idx, const Data* prediction_data, bool oob_prediction) {
  if (thread_ranges.size() > thread_idx + 1) {
    for (size_t i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i) {
...
void Forest::predictInternalInThread(unsigned int thread_idx) {
  // Creating thread ranges.
  std::vector<unsigned int>predict_ranges;
...
void Forest::computeTreePermutationImportanceInThread(unsigned int thread_idx, std::vector<double>& importance,
...
void Forest::loadFromFile(std::string filename) {
  ...
  // Skipping covariates' names (already read).
  unsigned int num_dependent_variables;
...
void Forest::loadDependentVariableNamesFromFile(std::string filename) {
  ...
  // Reading covariates' names.
  dependent_variable_names.clear();
  unsigned int num_dependent_variables = 0;
  infile.read((char*) &num_dependent_variables, sizeof(num_dependent_variables));
...
void Forest::setSplitWeightVector(std::vector<std::vector<double>>& split_select_weights) {
  ...
  for (size_t i = 0; i < split_select_weights.size(); ++i) {
    ...
    if (split_select_weights[i].size() != num_independent_variables) {
      throw std::runtime_error("Number of split select weights not equal to number of covariates.");
    }
...
void Forest::setAlwaysSplitVariables(const std::vector<std::string>& always_split_variable_names) {
  ...
  if (deterministic_varIDs.size() + this->mtry > num_independent_variables) {
    throw std::runtime_error(
        "Number of variables to be always considered for splitting plus mtry cannot be larger than number of covariates.");
  }
}
...
void Forest::showProgress(std::string operation, clock_t start_time, clock_t& lap_time) {
  ...
  if (elapsed_time > STATUS_INTERVAL) {
    ...
    unsigned int remaining_time = (1 / relative_progress - 1) * time_from_start;
...
void Forest::showProgress(std::string operation, size_t max_progress) {
  ...
  // Waiting for message from threads and show output if enough time elapsed.
  while (progress < max_progress) {
    ...
    if (progress > 0 && elapsed_time.count() > STATUS_INTERVAL) {
      ...
      unsigned int remaining_time = (1 / relative_progress - 1) * time_from_start.count();
