...
public:
  ...
  void init(const Data* data, unsigned int mtry, size_t num_samples, unsigned int seed, std::vector<size_t>* deterministic_varIDs,
      std::vector<double>* split_select_weights, ImportanceMode importance_mode, unsigned int min_node_size,
...
protected:
  ...
  // Value to split at for each node, for now only binary split. For terminal nodes the prediction value is saved here.
  std::vector<double> split_values;
  ...
  // When growing here the OOB set is used. Terminal nodeIDs for prediction samples.
